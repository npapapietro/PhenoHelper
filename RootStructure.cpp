...
namespace Representation {
	...
	weight weight::operator+(const weight & w)
	{
		this->ortho = this->ortho + w.ortho;
		this->omega = this->omega + w.omega;
		this->alpha = this->alpha + w.alpha;
	...
	*/
	...
	std::vector<weight> CircleGroup::tensorProductDecomp(weight w1, weight w2)
	{
		return std::vector<weight>{w1 + w2};
	}

	size_t CircleGroup::get_Rank()
	{
		return 0;
	}

	bool CircleGroup::is_singlet(weight w)
	{
		if (w.omega == Zero(1)) return true;
		else return false;
	}
