...
namespace Representation {
	...
	*/
	...
	Matrix<rational<int>, Dynamic, Dynamic> Zero(int size);
	...
	*/
    ...
    virtual	weight get_adjoint_rep() const = 0;

		virtual bool is_singlet(weight w) = 0;
	...
	*	U(1)
	*/
	class CircleGroup: public GroupBase
	{
		void createOrtho() override {};
		void createMatrices() override {};
		void createAllBases() override {};


		typedef Matrix <rational<int>, Dynamic, Dynamic> MatrixXr;
		typedef Matrix <rational<int>, Dynamic, 1> VectorXr;

		VectorXr to_alpha(weight V) override { return VectorXr(); };
		VectorXr to_omega(weight V) override { return VectorXr(); };
		VectorXr to_ortho(weight V) override { return VectorXr(); };


		std::vector<weight> weightTower() override { return std::vector<weight>(); };
		std::vector<weight> weightTower(weight w) override { return std::vector<weight>(); };

		MatrixXr get_Cartan() const { return MatrixXr(); };
		MatrixXr get_Omega() const { return MatrixXr(); };
		MatrixXr get_CoCartan() const { return MatrixXr(); };
		MatrixXr get_QuadraticForm() const { return MatrixXr(); };

		std::vector<weight> get_simple() const { return std::vector<weight>(); };
		std::vector<weight> get_positiver() const { return std::vector<weight>(); };
		std::vector<weight> get_fweight() const { return std::vector<weight>(); };

		int dim(weight w) { return 0; };
		int k_lvl(weight w) { return 0; };

		weight get_adjoint_rep() const { return weight(); };
public:
		CircleGroup() {}
		CircleGroup(const CircleGroup&) = default;
		CircleGroup(CircleGroup &&) = default;
		~CircleGroup(){}
		CircleGroup* clone() const { return new CircleGroup(*this); }
		CircleGroup& operator=(const CircleGroup&) = default;
		CircleGroup& operator=(CircleGroup&&) = default;

		size_t get_Rank();
		std::vector<weight> tensorProductDecomp(weight w1, weight w2);
		bool is_singlet(weight w);
	};
	...
	*/
	...
	public:
		...
		std::vector<weight> weightTower();
		std::vector<weight> weightTower(weight w);
		std::vector<weight> tensorProductDecomp(weight w1, weight w2);
	...
	template<GroupType T>
	inline bool LieBase<T>::is_singlet(weight w)
	{
		if (dim(w)==1) return true;
		else return false;
	}
...
*/
	...
	template<GroupType T>
	inline weight LieBase<T>::get_adjoint_rep() const
	{
		FILE_LOG(Diagnositics::logDEBUG3)<< "get_adjoint_rep called";
		return adjoint_rep;
	}
	template<GroupType T>
	inline void LieBase<T>::adjoint_rep_find(){
			std::vector<weight> algebra = weightTower();
			weight highest_weight = algebra[0];
			int k_c = k_lvl(algebra[0]);

			for (size_t i = 1; i < algebra.size(); i++)
			{
				int k_n = k_lvl(algebra[i]);
				if (k_n > k_c){
					k_c = k_n;
					highest_weight = algebra[i];
				}


			}
			adjoint_rep = highest_weight;
	}
